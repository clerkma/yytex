@echo off
echo Making TEXT font with extra composites %1.pfa (without remaking PFM)
...
rem echo fontone -vkl0 c:\cmbsr\%1
echo fontone -vDkl0 c:\cmbsr\%1
rem fontone -vkl0 c:\cmbsr\%1
fontone -vDkl0 c:\cmbsr\%1

:nofontone
rem Don't need new PFM if just adding extra composite
rem copydate -x c:\cmbsr\%1.pfm c:\cmbsr\%1.afm
rem if not errorlevel == 1 goto nopfmneed
rem afmtopfm -vsdt c:\cmbsr\%1.afm
