@echo off
rem Copyright (C) 1993, 1994 Y&Y, Inc.
...
rem NOTE: Make sure the encoding vector file you call for is (i) either in the
rem	  current directory or (ii) where the environment variable VECPATH
rem	  points, or (iii) give full path for vector file.

rem Check whether we have enough arguments!  Must have at least one font!

if not "%7" == "" goto lastok
echo Sorry, too few arguments
goto usage
:lastok
...
echo ERROR: Cannot find ATM.INI in %5
rem pause
goto abort
:atmok
...
rem
if "%TEXANSI%" == "" echo IMPORTANT: Add the line   set TEXANSI=1  to autoexec.bat 
...
echo Impossible error: no %7.pfb or %7_*.pfb file found
pause
goto immune
:dopfb
...
if "%6" == "W" decode -v -c=%1 -w=%5 %7
if "%6" == "WX" decode -v -c=%1 -w=%5 %7
...
echo ------------------------------------------------------------------------
echo 
echo WARNING: will now overwrite existing PFB, PFM and TFM files for %7
echo Do not proceed unless you have backup copies of PFB, PFM, and TFM files.
...
echo Copying reencoded %7_*.pfm back to %3\pfm
copy %7_*.pfm %3\pfm
:misspfmb
goto skippfm
...
echo Perhaps you are using an old version ATM?  Please upgrade to ATM 2.5
:gone
...
echo       W          write modified files back to their directories
echo       N          modified files appear only in current directory
...
echo The output will also overwrite the original files if  W  is specified.
echo Do not run the %0 batch file from inside Windows when using flag W.
