	...
	if ( ( readperformed ) ) 
	{
	  ...
	  } 
	  {
		...
		while ( ( entrystrs [ strentptr ] [ entchrptr ] != 127 ) ) {
		  ...
		  {
		    putc ( xchr [ entrystrs [ strentptr ] [                     entchrptr ] ] ,  logfile );
		  } 
...
{poolpointer pptr  ; 
  ...
  {
    {
      fprintf( logfile , "%s%c%s",  "File=" , nameoffile , ", extension=" ) ; 
...
{poolpointer pptr  ; 
  ...
  ) ) 
  {
    ...
    printapoolstr ( area ) ; 
    {
      fprintf( logfile , "%c%c",  nameoffile , ',' ) ; 
      fprintf( standardoutput , "%c%c",  nameoffile , ',' ) ; 
    } 
    filenmsizeoverflow () ; 
  } 
  nameptr = namelength ; 
...
void 
zinttoASCII ( theint , intbuf , intbegin , intend ) 
  integer theint ; 
  buftype intbuf ; 
  bufpointer intbegin ; 
  bufpointer * intend ; 
...
{/* 25 10 */ typedef integer fndefloc  ; 
  ...
  } 
  {
    ...
    {
      {
	fprintf( logfile , "%ld%s",  (long)singleptr + wizdefptr , ": " ) ; 
	fprintf( standardoutput , "%ld%s",  (long)singleptr + wizdefptr , ": " ) ; 
      } 
      {
	printoverflow () ; 
	{
...
{/* 41 46 */ getoptstruct longoptions[noptions + 1]  ; 
  ...
  while ( true ) {
    ...
    if ( ( argc > 1 ) ) 
    {
      if ( argc > noptions + argoptions + 3 ) 
      {
	{
	  Fputs( logfile ,  "Usage: bibtex " ) ; 
	  Fputs( standardoutput ,  "Usage: bibtex " ) ; 
	} 
	{
	  Fputs( logfile ,  "[-verbose] " ) ; 
	  Fputs( standardoutput ,  "[-verbose] " ) ; 
	} 
	{
	  Fputs( logfile ,  "[-min-crossrefs=<number>] " ) ; 
	  Fputs( standardoutput ,  "[-min-crossrefs=<number>] " ) ; 
	} 
	{
	  fprintf( logfile , "%s\n",  "[<aux file>]." ) ; 
	  fprintf( standardoutput , "%s\n",  "[<aux file>]." ) ; 
	...
	longoptions [ currentoption ] .val = 0 ; 
	do {
	    getoptreturnval = getoptlongonly ( argc , gargv , "" , 
	  longoptions , addressofint ( optionindex ) ) ; 
      ...
      Fputs( standardoutput ,  "Please type input file name (no extension)--"       ) ; 
      auxnamelength = 0 ; 
...
{/* 22 26 15 10 */ atbibcommand = false ; 
  ...
  } 
  {
      ...
      case 1 : 
	{
	  ...
	  {
	    {
	      fprintf( logfile , "%s%ld%s",  "You've exceeded " , (long)maxbibfiles ,               " preamble commands" ) ; 
	      fprintf( standardoutput , "%s%ld%s",  "You've exceeded " , (long)maxbibfiles ,               " preamble commands" ) ; 
	    } 
	    biberrprint () ; 
	    goto lab10 ; 
	  } 
...
{/* 10 */ if ( ( readseen ) ) 
  ...
  } 
  {
	...
	{
	  {
	    fprintf( logfile , "%ld%s",  (long)numentints * numcites , ": " ) ; 
	    fprintf( standardoutput , "%ld%s",  (long)numentints * numcites , ": " ) ; 
	  } 
	  {
	    printoverflow () ; 
	    {
	...
	{
	  {
	    fprintf( logfile , "%ld%s",  (long)numentstrs * numcites , ": " ) ; 
	    fprintf( standardoutput , "%ld%s",  (long)numentstrs * numcites , ": " ) ; 
	  } 
	  {
	    printoverflow () ; 
	    {
