...
double skew=0.0;		/* skew factor for oblique font */ /* 1992/Aug/25 */
...
double xscaleinit=1.0;		/* global scale - 1993/Jan/1 */
double yscaleinit=1.0;		/* global scale - 1993/Jan/1 */
	...
	while(strstr(inline, "end def") == NULL) {
		if (strstr(inline, "/.notdef") == NULL) {
			if (sscanf(inline, " /char%d %d def", &fchrs, &width) != 2) {
				fprintf(stderr, "Not character width info: %s\n", inline);
				giveup(3);
			}
			...
			if (fchrs >= 0 && fchrs < fontchrs) widths[fchrs] = width;
				...
				fprintf(stderr, "Character number %d out of range\n", fchrs);
				giveup(7);
			}
		}
		(void) getline(fp_in, inline);
...
/*			if (k == 0) break;  */
			...
			if (sscanf(inline, "%d %d", &chr, &width) < 2) {
				fprintf(stderr, 
					"Character number or width bad: %s (%d) ", inline, k); 
				if (getline(fp_in, inline) == 0)
					fprintf(stderr, "EOF!\n");
				else fprintf(stderr, "Next line: %s", inline);
				giveup(11);
			}
...
/*	printf("RUSTICFLAG %d\n", rusticflag); */
	...
	if (rusticflag != 0) {
		...
		if (strstr(inline, "dict") == NULL) {
			fprintf(stderr, "First line is not font dict: %s\n", inline);
			giveup(2);
		}
...
/*		if (scalef == 2.048 && pctexbug != 0) scalef = 2.005; */
		if ((s = strstr(inline, "scale")) != NULL) {	/* 1993/Jan/1 */
			if (sscanf(s, "scale %lg %lg", &xscaleinit, &yscaleinit) < 2)
				yscaleinit = xscaleinit;
		}
		if (ptsize == 0) {
		...
		if (adobescaling == 0 ) {
			if (scaleup != 0) fprintf(stderr, "WARNING: scaling up! ");
			if (verboseflag != 0 || scaleup != 0) {
				if (correctscale == 0) 	fprintf(stderr, "Scaling by %lg\n",
		...
		if (sscanf(inline, "%d %d %d %d", &xll, &yll, &xur, &yur) != 4) {
			fprintf (stderr, "Second line not FontBBox: %s", inline);
			giveup(7);
		}
		if (xll == xur || yll == yur) {	/* emergency defaults */
			fprintf(stderr, "Bad FontBBox: %s", inline);
			xll = -50; yll = -250; xur = 1000; yur = 1000;
		}
		xll = scalenum(xll); yll = scalenum(yll);
		xur = scalenum(xur); yur = scalenum(yur);
		if (verboseflag != 0)
...
void closepath(FILE *fp_out) {
	if (xlast != xstart || ylast != ystart) {	/* new check */
		if (complainopen != 0 || traceflag != 0) 
			fprintf(stderr, "Subpath not closed in char %d\n", chrs);
...
/* void moveto(FILE *fp_out, int xoff, int yoff) { */
void moveto(FILE *fp_out) {
	double fx, fy;
	int x, y;
	if (closed == 0) {
		fprintf(stderr, "Subpath in char %d not closed\n", chrs);
		fatal++;		/* ??? */
	}
/*	if (sscanf(inline, " %d %d moveto", &x, &y) != 2) { */
	if (fixonly) {
		if (sscanf(inline, " %d %d m", &x, &y) != 2) {
			fprintf(stderr, "Invalid moveto: %s", inline);
			giveup(12);
		}
	}
	else {
...
/* void lineto(FILE *fp_out, int xoff, int yoff) { */
...
/*	if (sscanf(inline, " %d %d lineto", &x, &y) != 2) { */
	if (fixonly) {
		if (sscanf(inline, " %d %d l", &x, &y) != 2) { 
			fprintf(stderr, "Invalid lineto: %s", inline);
			giveup(12);
		}
	}
	else {
...
void oval (FILE *fp_out) {
	...
	if (fixonly) {
		if (sscanf(inline, " %d %d %d %d o",
			&xa, &ya, &xb, &yb) != 4) { 
			fprintf(stderr, "Invalid oval: %s", inline);
			giveup(12);
		}
	}
	else {
	...
	xa = xa + xoff; ya = ya + yoff;
	xb = xb + xoff; yb = yb + yoff;
	if (xscale != 1.0 || yscale != 1.0) {
/*		x = round (scale * x); y = round (scale * y); */
	...
	if (rusticflag == 0) {
		xold = xb, yold = yb;
		xa = scalenum(xa), ya = scalenum(ya);
		xb = scalenum(xb), yb = scalenum(yb);
	}
	xc = xb; yc = yb;
...
/*	int xa, ya, xb, yb, xc, yc; */
	if (fixonly) {
		if (sscanf(inline, " %d %d %d %d x",
			&xll, &yll, &xur, &yur) != 4) { 
			fprintf(stderr, "Invalid circle: %s", inline);
			giveup(12);
		}
	}
	else {
	...
	xll = xll + xoff; yll = yll + yoff;
	xur = xur + xoff; yur = yur + yoff;
	if (xscale != 1.0 || yscale != 1.0) {
/*		x = round (scale * x); y = round (scale * y); */
...
/*	if (za == zb) {
	...
	ic = scalenum(zc);	 
/*	assert(ia == ib); */
	if (ia != ib) fprintf(stderr, "ERROR: ia != ib (%d != %d)\n", ia, ib);
	if (ib == ic) {
		if (traceflag != 0)		/* verboseflag */
...
/*	assert(scalenum(zbd) == ib); */
	if (scalenum(zbd) != ib)
		fprintf(stderr, "scalenum(zbd) != ib (%d != %d)\n",
			scalenum(zbd), ib);
/* 	assert(zbd != zb); */
	if (zbd != zb && adobescaling != 2)
		fprintf(stderr, "NOTE: zbd != zb (%d != %d)\n", zbd, zb);
	if (traceflag != 0)	/* verboseflag */
...
/*	int ia, ib, ic; */
	...
	if (adjustknots == 0) return zb;
/*	assert(za == zb); */
	if (za != zb) fprintf(stderr, "za != zb (%d != %d)\n", za, zb);
	if (zc == zb) {
		if (verboseflag != 0)
...
/*	assert(zc != zb); */
	...
	if (zbd == zc) {
		fprintf(stdout, "\nIn char %d Can't adjust: ", fchrs);
		if (flag != 0) fprintf(stdout, "(%d %d) ", ub, zb);
		else fprintf(stdout, "(%d %d) ", zb, ub);
		fprintf(stdout, "(%d %d %d) ", za, zb, zc); /* stderr ? */
/*		fprintf(stderr, "\a");  (void) _getch(); */
		zbd = zb;
	}
/*	assert(zbd != zc); */
	if (traceflag != 0)		/* verboseflag */
...
/* void curveto(FILE *fp_out, int xoff, int yoff) { */
void curveto(FILE *fp_out) {
	...
	if (fixonly) {
		if (sscanf(inline, " %d %d %d %d %d %d c",  
			&xa, &ya, &xb, &yb, &xc, &yc) != 6) {
			fprintf(stderr, "Invalid curveto: %s", inline);
			giveup(12);
		}
	}
	else {
	...
	xa = xa + xoff; ya = ya + yoff;	 
	xb = xb + xoff; yb = yb + yoff;		
	xc = xc + xoff; yc = yc + yoff;		
	if (xscale != 1.0 || yscale != 1.0) {
/*		xa = round (xscale * xa); ya = round (yscale * ya); */
		xa = xinscale (xa); ya = yinscale (ya);
/*		xb = round (xscale * xb); yb = round (yscale * yb); */
		xb = xinscale (xb); yb = yinscale (yb);
/*		xc = round (xscale * xc); yc = round (yscale * yc); */
...
/* void longcurveto(FILE *fp_in, FILE *fp_out, int xoff, int yoff) { */
void longcurveto(FILE *fp_in, FILE *fp_out) {
	int xa, ya, xb, yb, xc, yc;
	if (sscanf(inline, " %d %d", &xa, &ya) != 2) {
		fprintf(stderr, "Invalid potential long curveto (1): %s", inline);
		if (getline(fp_in, inline) == 0)
			fprintf(stderr, "EOF!\n");
		else fprintf(stderr, "Next line: %s", inline);
		giveup(12);
	...
	xa = xa + xoff; ya = ya + yoff;	
	if (xscale != 1.0 || yscale != 1.0) {
/*		xa = round (scale * xa); ya = round (scale * ya); */
		xa = xinscale (xa); ya = yinscale (ya);
	}
	(void) getline(fp_in, inline);
	if (sscanf(inline, " %d %d", &xb, &yb) != 2) {
		fprintf(stderr, "Invalid potential long curveto (2): %s", inline);
		if (getline(fp_in, inline) == 0)
			fprintf(stderr, "EOF!\n");
		else fprintf(stderr, "Next line: %s", inline);
		giveup(12);
	...
	xb = xb + xoff; yb = yb + yoff;		
	if (xscale != 1.0 || yscale != 1.0) {
/*		xb = round (scale * xb); yb = round (scale * yb); */
		xb = xinscale (xb); yb = yinscale (yb);
	}
	(void) getline(fp_in, inline);
	if (sscanf(inline, " %d %d c", &xc, &yc) != 2) {
		fprintf(stderr, "Invalid potential long curveto (3): %s", inline);
		if (getline(fp_in, inline) == 0)
			fprintf(stderr, "EOF!\n");
		else fprintf(stderr, "Next line: %s", inline);
		giveup(12);
	...
	xc = xc + xoff; yc = yc + yoff;		
	if (xscale != 1.0 || yscale != 1.0) {
/*		xc = round (scale * xc); yc = round (scale * yc); */
		xc = xinscale (xc); yc = yinscale (yc);
	}
	if (rusticflag != 0 && correctscale != 0) { /* new */
		xc = round(((double) xc) * 72.27 / 72.0); 
...
void finishchar(FILE *fp_out) {
	if (xlast == INFINITY || ylast == INFINITY) { /* empty ? */
		if (complainopen != 0 || traceflag != 0) 
			fprintf(stderr, "Empty character %d\n", chrs);
/*		nputc('D', fp_out);		*/
		writetext("AD", fp_out);		/* fake closepath */
	}
	if (closed == 0) {
		fprintf(stderr, "\nLast path in char %d not closed\n", chrs);
		if (closelastpath) {
/*			current = 0; */
...
/* void dochar(FILE *fp_in, FILE *fp_out, int fchrs, int xoff, int yoff) { */
...
/*	int xoff=0, yoff=0; */
	...
	xoff = 0; yoff = 0;		/* xscale = 1.0; yscale = 1.0;  */
...
/*				strchr(inline, 'm') != NULL)) {*/
			...
			if (allowmovetoin) {
				...
				if ((s = strstr(inline, "moveto")) != NULL)	strcpy(s, "lineto");
				else if ((s = strchr(inline, 'm')) != NULL) *s = 'l'; 
				fatal++;
			}
		}
...
/*				if (sscanf(s, "offset %d %d %lg", &xoff, &yoff, &scale) < 2) { */
				xoff = 0; yoff = 0;					/* 1992/Sep/12 */
				if (sscanf(s, "offset %d %d", &xoff, &yoff) < 2) { 
					fprintf(stderr,
						"ERROR: Don't understand offset %s", inline);
					fatal++;	/* ??? */
				}
				if (verboseflag != 0 && showoffsets != 0) {
/*					if (xscale != 1.0 || yscale != 1.0)  */
...
/*			strchr(inline, 's') != NULL) *//* new, for hint replacement */
			strstr(inline, " s") != NULL) /* new, for hint replacement */
			callsubr(fp_out); 
		...
		while (strstr(inline, "end def") == NULL) {
			if (strstr(inline, "/.notdef") == NULL) {
				if (sscanf(inline, " /char%d {", &fchrs) != 1) {
					fprintf(stderr, "Bad character definition: %s\n", inline);
					giveup(13);
				}
				if (verboseflag != 0) {
					printf(".");
...
/*			scale = 1.0;
			if ((s = strstr(inline, "scale")) != NULL) {
				if (sscanf(s, "scale %lg", &scale) < 1)
					fprintf("UGH");
				widthf = (int) (widthf * scale + 0.5);
			} 
			else s = inline; */
			...
			if ((s = strstr(inline, "scale")) != NULL) {
				...
				if (traceflag) fputs(inline, stdout);	/* debugging */
				if (n < 1) {
					fprintf(stderr, "ERROR: Don't understand %s", inline);
					fatal++;	/* ??? */
				}
...
/*				widthf = (int) (widthf * xscale + 0.5);	*/		/* ??? */
			...
			if ((s = strchr(s, '%')) != NULL) 
				fputs(s, fp_out);			/* copy comment across - NEW */
...
/*		if (strlen(inline) > 0) {
			fprintf(stderr, "Character data does not start with ']': %s", 
				inline);
			giveup(17);
		} */
	}
...
/*				printf("First line %s", inline); (void) _getch(); */
				...
				fprintf(stderr, "ERROR: Unable to find character %d\n", chrs); 
				fatal++;
				fseek(fp_in, fstart, SEEK_SET);
				chrs = nxtchrs - 1;					/* ??? */
				break;
...
/*				else fputs(inline, stdout);	 */		/* debugging */
				...
				dochar(fp_in, fp_out, fchrs);
/*				if (getline(fp_in, inline) == 0) {
					fprintf(stderr, "Unexpected EOF"); giveup(1);
				} */
				while (*inline == '%' || *inline == '\n') 
					(void) getline(fp_in, inline);
				break;
...
/*		(void) _getch(); */
	...
	fprintf(fp_out, "\npop\n");
	xmin = scalenum(xmin); ymin = scalenum(ymin); /* ? */
	xmax = scalenum(xmax); ymax = scalenum(ymax); /* ? */
...
/*	if (verboseflag != 0) */
	if (xmin == INFINITY || xmax == -INFINITY ||
		...
		printf("WARNING: Actual FontBBox %d %d %d %d\n", 
			xmin, ymin, xmax, ymax);
}
...
/*	fprintf (stderr, "\tx: next argument is version of font\n"); */
	fprintf (stderr, "\tx: global scale in x\n");
	fprintf (stderr, "\ty: global scale in y\n");
	fprintf (stderr, "\tw: complain if sub-paths are not closed\n");
	fprintf (stderr, "\tz: complain if side-slip off vertical/horizontal\n");
/*	fprintf (stderr,
	"\te: next argument is extension to use on output file (default '%s')\n", 
		ext); */
/*	fprintf (stderr,
	"\tp: next argument is path to use on output file");
	if (strcmp(path,"") != 0) fprintf(stderr, " (default is '%s')", path);
	fprintf(stderr, "\n"); */
	exit(1);
}
...
/* Command line usage: */
    FILE *fp_in, *fp_out;
    char fn_in[FNAMELEN], fn_out[FNAMELEN];
	char *s;
	unsigned i;
	int c, m, firstarg=1;
...
/*		if (rusticflag == 0) { */ 
			...
			if ((s = strchr(fontname, '.')) == NULL) {
				fprintf(stderr, "Don't understand fontname: %s", fontname);
				giveup(13);
			}
			*s = '\0';
