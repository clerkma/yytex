...
// char *outfile1="image001s.bmp";

char *infile3="image003.bmp";
// char *outfile3="image003s.bmp";
...
double iicoords[65][85][2];	// BB image center coordinates
...
int make_affine_matrix (double m[3][3]) {
	int i, j, count = 0;
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) {
			m[i][j] = 0.0;
...
int affinefitx (double c[3]) {
	...
	count = 0;
	if (make_affine_matrix(m)) return -1;
